...
function performSearch(recordType, batchSize, lowerBound, rawFilters, rawColumns) {
    ...
    for(var filter in rawFilters) {
        ...
        if(rawFilters[filter].hasOwnProperty('formula')) {
            searchFilters[searchFilters.length-1].setFormula(rawFilters[filter]['formula']);
        }
    }
...
// Goes through sublist, skipping sublist items whose values don't
//  match what is expected
function setSlist(record, slistType, h) {
    var cnt = record.getLineItemCount(slistType);
    for(var i = 1; i <= cnt; ++i) {
	record.selectLineItem(slistType, i);
	var matches = true;
	for(var k in h) {
	    var slist_val = record.getLineItemValue(slistType, k, i);
	    if(slist_val != h[k])
		matches = false;
	}
	if(matches)
	    return record.selectLineItem(slistType, i);
    }
}
...
function updateSublist(recordType, hash) {
	...
	var slistArr  = hash['sublist_fields'][slistType];
	for(var i = 0; i < slistArr.length; ++i) {
	    var h = slistArr[i];
	    setSlist(record, slistType, h);
	    for(var k in h)
...
function upsertRecords(request) {
    ...
    for(var index = 0; index < recordData.length; index++) {
        ...
        try {
            if(staticSublist == 'T') {
                var res = updateSublist(recordType, recordData[index]);
                writeResults.push([res])
                if((index + 1) == recordData.length)
                    return res;
                else
                    continue;
            }
...
function transformRecords(request) {
    ...
    var newRecord = nlapiTransformRecord(initialRecordType, internalId, resultRecordType);
    ...
    for(var sublistName in sublistChanges) {
        var matchField      = sublistChanges[sublistName]['match_field'];
        var sublistItems    = sublistChanges[sublistName]['line_items'];
        var indexesToDelete = [];
        ...
        for(var lineItemIndex = 1; lineItemIndex < (newRecord.getLineItemCount(sublistName) + 1); lineItemIndex++) {
            ...
            for(var sublistItemDataIndex = 0; sublistItemDataIndex < sublistItems.length; sublistItemDataIndex++) {
                var sublistItemData = sublistItems[sublistItemDataIndex];
        ...
        for(var index = 0; index < indexesToDelete.length; index++) {
            var deletionIndex = indexesToDelete[index];
            if (resultRecordType === 'itemreceipt') { //Refactor to a callback function?
              newRecord.setLineItemValue(sublistName, 'itemreceive',(deletionIndex - deletionCount), 'F');
            } else {
    ...
    return([nlapiSubmitRecord(newRecord, false, false), [internalId, fieldChanges, sublistChanges], {log: log}]);
}
